...
class Options extends Common
{
    ...
    {
        return Stock::find()->where(['id'=>12])->with(['images','gems','d3_files'])->asArray()->one();    
    }
    public function writeToFile()
    {
        $dat = $this->getTestData();

        $datEnc = json_encode($dat);
        //debug($datEnc,'$datEnc',1);

        // Вначале убедимся, что файл существует и доступен для записи.
        $filename = _webDIR_ . "backup/testModel.json";
        //debug($filename,'$filename',1);
        if ( is_writable($filename) ) {
            ...
            if (fwrite($fp, $datEnc) === FALSE) {
                echo "Не могу произвести запись в файл ($filename)";
                exit;
            }

            echo "Ура! Записали ($datEnc) в файл ($filename)";
